...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& {
    ...
    $batContent = '@echo off' + [Environment]::NewLine + 'chcp 65001 >nul' + [Environment]::NewLine + 'title EFD 眼睛特徵提取與即時疲勞監控研究系統' + [Environment]::NewLine + 'cd /d ""%~dp0""' + [Environment]::NewLine + 'echo 正在啟動 EFD 即時視覺與疲勞監控核心...' + [Environment]::NewLine + 'echo 關閉介面後本視窗將自動最小化至工作列，並在背景持續輸出最新眼動數據。' + [Environment]::NewLine + 'echo.' + [Environment]::NewLine + '""%~dp0EFD.exe""' + [Environment]::NewLine + 'if %ERRORLEVEL% neq 0 pause' + [Environment]::NewLine;
